class Book < ApplicationRecord
  ...
  def get_settings
    ...
    if reset
      ...
      ns = {}.with_indifferent_access
      accts = build_tree
      id_trans = accts.pluck(:id,:transfer)
      if checking.present?
        ns[:checking_acct_id] = checking.id 
        ns[:checking_ids] = checking.leaf
      end
      ns[:transfers] = id_trans.to_h
      ns[:tree_ids] = ns[:transfers].keys
      ns[:acct_sel_opt] = id_trans.map{|i| i.reverse}.prepend(['',0])
      ns[:dis_opt] = accts.select{|a| a.placeholder}.pluck(:id)
      ns[:acct_sel_opt_rev] = ns[:acct_sel_opt].
        select{|i| i  unless ns[:dis_opt].include?(i[1])}.
